            ...
            <div className="flex flex-wrap sm:max-w-9/11">
              {
               articleLength.map((item,index)=>(
                ...
                onClick={()=>{setSelectedLength(item)}}
                 className={`text-xs   m-1 rounded-full  px-4 py-1 border  ${selectedLength.text==item.text ? 'bg-blue-50 text-blue-700 border  border-gray-600':'text-gray-500 border border-gray-300'}`} 
                 key={index}
                 >{item.text}</span>
               ))
              }
