...
export const generateImage = async(req,res)=>{
    try{
       ...
       if(plan!=='premium' ){
        return res.json({
            success:false,
        message:"This feature is available at premium only"
        })
       }
...
res.json({success:true,secure_url})
    ...
    details: error.response?.data?.toString() || error.code || null
      })
    }
}
...
export const removeImageBackground = async (req, res) => {
  try {
    ...
    const uploadResult = await new Promise((resolve, reject) => {
      ...
      stream.end(image.buffer); // send buffer data
    });
...
export const removeImageObject = async (req, res) => {
  try {
    ...
    const uploadResult = await new Promise((resolve, reject) => {
      ...
      stream.end(image.buffer); // pass buffer here
    });

    // 2. Apply transformation for object removal
